    ...
    public class Bdd
    {
        public async Task<List<Appointment>> LoadRendezVousAsync(int commercialId)
        {
            ...
            using (var conn = new MySqlConnection(connectionString))
            {
                ...
                string query = @"SELECT a.*, c.Lastname AS ClientLastName 
                         FROM appointment a 
                         LEFT JOIN client c ON a.ID_Client = c.ID_Client 
                         WHERE a.ID_Commercial = @CommercialId";
        ...
        public async Task<Commercial> GetCommercialByEmailAndPasswordAsync(string email, string password)
        {
            ...
            {
                await conn.OpenAsync();
                using (var cmd = new MySqlCommand("SELECT * FROM commercial WHERE Email = @Email", conn))
                {
                    ...
                    using (var reader = await cmd.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            ...
                            string computedHash = Sécurité.HashPassword(password); // Use la méthode HashPassword de la classe Sécurité
                            if (computedHash.Equals(storedPasswordHash))
                            {
